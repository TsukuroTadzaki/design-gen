# Task: Rewrite Generation Orchestrator

## What changed

Old approach: bash script injects full document content into `claude -p` prompt → broke on escaping, quotes, long text.

New approach: short `claude -p` prompts that tell Claude to READ files itself and SAVE output. Each prompt is 5-10 lines. Claude handles file I/O.
...
if [ -z "$PROJECT_ID" ]; then
  echo "Usage: ./scripts/generate.sh {project-id}"
  exit 1
fi
...
# ──────────────────────────────────────────
# STEP 4: All sections from plan.json
# ──────────────────────────────────────────

echo ""
echo "── Step 4: Sections ──"
...
if type_count > 1:
    # Multiple of same type — use full id (home-hero → HomeHeroSection)
    name = ''.join(p.capitalize() for p in parts) + 'Section'
else:
    # Unique type — skip page prefix (home-hero → HeroSection)
...
"Read $PLAN_FILE — find the section with id '$SECTION_ID'. Note its pattern_id, pattern_name, background, content_source, why, and notes.
...
PAGE_SLUGS=$(cat "$PROJECT_DIR/structure.json" | python3 -c "
import json, sys
...
for SLUG in $PAGE_SLUGS; do
  PASCAL_NAME=$(python3 -c "print(''.join(p.capitalize() for p in '$SLUG'.split('-')))")
  PAGE_FILE="$OUTPUT_DIR/pages/${PASCAL_NAME}.tsx"

  generate "$PASCAL_NAME.tsx" \
"Read $PROJECT_DIR/structure.json — find the page with slug '$SLUG', get its sections list (in order).
Look at the actual files in $OUTPUT_DIR/sections/ to match section IDs to file names.

Generate a page component named $PASCAL_NAME that:
- Imports each section from '../sections/{FileName}'
...
Read $PROJECT_DIR/structure.json — get all pages (slugs and names).
...
- Imports each page component from './pages/{Name}'
- Has routes array: { path: '/', name: 'Home', component: Home } for home, { path: '/{slug}', name: '{Name}', component: {Component} } for others
...
|---|---|
| Injected full file contents into bash prompt | Claude reads files itself |
| 200+ line prompt with escaping hell | 10-15 line prompt with file paths |
| Bash handled output parsing | Claude saves files directly |
| Markdown fences in output | Claude writes .tsx files natively |

## Setup

```bash
# Create the script
# (paste the script above into scripts/generate.sh)

# Make executable
chmod +x scripts/generate.sh

# Test on KavaBAR (clean slate)
...
# Keep styles.css if good, or delete to regenerate
...
# Re-run after fixing one section (skips existing)
./scripts/generate.sh kava

# Force regenerate specific section
rm src/projects/kava/output/sections/HeroSection.tsx
./scripts/generate.sh kava

# Force regenerate everything
rm -rf src/projects/kava/output/sections/* src/projects/kava/output/pages/*
./scripts/generate.sh kava
```

## Notes

- `sleep 2` between calls prevents rate limiting — increase to `sleep 5` if getting 429 errors
- Script uses `python3` for JSON parsing and PascalCase conversion — ensure python3 is available
- Each `claude -p` call is fully isolated — no context leaks between sections
- Failed sections (file not created) are counted as errors — re-run to retry
